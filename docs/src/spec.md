...
azoth has **38 ids** — 21 calculations and 17 models.

## The ledger

`databank/manifest.yaml` records every column of every vendored NeqSim table, and for each
column that is not carried across, the NeqSim class that would close it. It is checked by
`tools/check_manifest.py`, which prints the tally, and the check refuses a reason with no
NeqSim name in it.

**That tally is the port's progress bar.** It goes one way. As of the last commit it reads:

```
  139  not-ported        a model NeqSim has and this library does not
   21  not-yet           the model exists here and the data does not reach it
   10  not-a-value       an identifier, index or selector, not a property
   22  empty-upstream    nothing in the source to carry
    5  superseded-by      another column says the same thing
```
