<!-- Generated by tools/gen_docs.py from specs/models/eos/ps_flash.toml. Do not edit by hand. -->

# Pressure-entropy flash

`eos.ps_flash`

## Source

**Standard thermodynamics, as in Smith, Van Ness & Abbott; Michelsen & Mollerup**

The procedure is the ordinary one: the entropy of a mixture at fixed pressure is a strictly increasing function of temperature.


## Algorithm

A model rather than a calculation: what this page pins down is the procedure,
not an equation, and both implementations read it from here.

| Setting | Value |
|---|---|
| Scheme | `ps_flash_temperature_newton` |
| Convergence | `absolute` |
| Tolerance | `1e-05` |
| Max iterations | `200` |

## Inner procedure

| Setting | Value |
|---|---|
| Scheme | `successive_substitution_flash` |
| Convergence | `absolute` |
| Tolerance | `1e-10` |
| Max iterations | `300` |
| Initialisation | `wilson` |

### Inner procedure

| Setting | Value |
|---|---|
| Scheme | `rachford_rice_bisection` |
| Convergence | `absolute` |
| Tolerance | `1e-14` |
| Max iterations | `200` |

## Inputs

| Name | Unit | Description |
|---|---|---|
| `components` | - | the substances the mixture is made of, by name, resolved against the component databank (`data/components/`, from NeqSim's COMP.csv and INTER.csv) with the keycard's overrides applied |
| `P` | Pa | absolute pressure. Held fixed; the temperature is what is solved for. |
| `S` | J/(mol*K) | the molar entropy the mixture is to reach, as a difference from the datum above. A magnitude, and it may be negative - a reference state that puts zero anywhere is a convention, not a physical floor. |
| `z` | dimensionless | the feed's mole fractions. Checked rather than renormalised. |


## Outputs

| Name | Unit | Description |
|---|---|---|
| `T` | K | the temperature that satisfies the entropy. This is the answer. |
| `beta` | dimensionless | the vapour fraction at that temperature. Absent for a single-phase feed, where the flash reports an extrapolated split rather than a physical one - see the notes. |
| `x` | dimensionless | liquid-phase mole fractions at the answer |
| `y` | dimensionless | vapour-phase mole fractions at the answer |
| `k` | dimensionless | `K_i = y_i / x_i` at the answer, reported because the flash is one |
| `phase` | all_liquid / two_phase / all_vapour | which phase the feed is in at the answer |
| `z_liquid` | dimensionless | the liquid root of the cubic at the answer |
| `z_vapour` | dimensionless | the vapour root |
| `iterations` | dimensionless | Newton iterations taken |
| `residual` | J/(mol*K) | `|S(T) - S_target|` at the answer, in the property's own unit |

| Bound | On violation | Why |
|---|---|---|
| `P > 0` | raises | an absolute pressure; zero and below are not states, and the cubic's reduced variables divide by it. |

## Assumptions

- the iteration is quasi-Newton from `initial_temperature`, not a scan: the step is `factor * residual / (dS/dT)` with `dS/dT = cp/T`, clamped to ten kelvin.
- the tolerance is `max(1e-5, |S_target| * 1e-10)`, in J/(mol*K): the floor is upstream's, stated per mole because this model carries no flow.
- the starting temperature is declared in the spec, where upstream reads one off its thermodynamic system.
- the flash at each trial temperature is converged, not exact: the entropy inverted is that of a converged approximation.
- no check that the feed is stable: the model asks where the energy balance is satisfied, not whether the state found is the equilibrium one.
- **a pure component's two-phase state is not found**, for the reason `eos.ph_flash` records: the temperature there is the saturation temperature. Measured on propane at 5 bar it converges at `S <= -100` and fails above. `PSflashSingleComp` is not ported.
- **the entropy is a difference from a supplied datum.** It is not an absolute quantity and is not comparable with a value computed from a different reference state.
- NeqSim carries a second S-flash scheme, `SysNewtonRhapsonPHflash` with its entropy residual, which `QfuncFlash` selects. It is not ported: `PhFlashJacobianProbe.java` measures the same two-phase factor of two on `dS/dT = Cp/T`.

## Cases

| Case | Inputs | Expected |
|---|---|---|
| `two_phase_round_trip` | components = ['methane', 'n-butane'], P = 2000000.0, S = -33.37924808318274, z = [0.6, 0.4] | T = 300.00000000000006, beta = 0.6824887179287709 |
| `single_phase_vapour_round_trip` | components = ['methane', 'n-butane'], P = 2000000.0, S = 4.29557591174716, z = [0.6, 0.4] | T = 400.0 |

## References

- Michelsen, M. L.; Mollerup, J. M. (2004). "Thermodynamic Models: Fundamentals and Computational Aspects." Tie-Line Publications.
- Smith, J. M.; Van Ness, H. C.; Abbott, M. M. "Introduction to Chemical Engineering Thermodynamics."
