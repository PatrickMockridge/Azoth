...
- `h_res`/`s_res` are `Z - 1 - T d(A^R/(RT))/dT` at constant volume, differentiated analytically, and `PhaseSAFTVRMie.dF_HC_SAFTdT` is the oracle for them since `#3830` added the chain contact value's temperature route. No `cp_res`: the second derivative is not derived.
