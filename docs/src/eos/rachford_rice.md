...
`RachfordRice.calcBetaNielsen2023` from NeqSim master, minus the single-phase dispatch: NeqSim answers a root outside `[0, 1]` with the `phaseFractionMinimumLimit` clamp, and this returns the root.
...
- NeqSim's `RachfordRice` carries two solvers behind a static `method` field. This ports `calcBetaNielsen2023`, the default: nothing in NeqSim calls `setMethod`, so every flash in the library runs it. `calcBetaMichelsen2001` is reachable only directly, and is named here rather than ported.
