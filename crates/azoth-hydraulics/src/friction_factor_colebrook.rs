...
///
/// # Errors
/// * [`azoth_core::AzothError::OutOfRange`] if `Re <= 0` (the `2.51/(Re*sqrt(f))`
///   term is singular) or if `relative_roughness < 0` (unphysical).
/// * [`azoth_core::AzothError::SolverNotConverged`] if the iteration hits its
...
/// # Example
/// ```
/// use azoth_hydraulics::friction_factor_colebrook;
///
