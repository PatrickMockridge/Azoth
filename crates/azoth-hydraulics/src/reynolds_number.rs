...
use crate::spec_gen;
use azoth_core::units::{DynamicViscosity, Length, MassDensity, Velocity};
use azoth_core::{FlowRegime, Result, Warning, apply_checks};
...
/// [`azoth_core::WarningCode::TransitionalFlow`] warning, because the friction
/// factor - and therefore any pressure drop derived from it - is indeterminate
/// in that band rather than merely uncertain.
///
/// # Errors
/// Returns [`azoth_core::AzothError::OutOfRange`] if density, diameter or
...
/// use azoth_hydraulics::reynolds_number;
///
...
pub fn regime_warning(regime: FlowRegime) -> Option<Warning> {
    regime.is_indeterminate().then(|| {
        Warning::new(
            azoth_core::WarningCode::TransitionalFlow,
            format!(
