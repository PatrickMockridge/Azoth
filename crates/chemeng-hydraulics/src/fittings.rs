...
fn parse() -> Result<Vec<Fitting>> {
    ...
    for record in reader.records() {
        let record = record.map_err(|e| {
            ChemEngError::invalid_input("fittings", format!("malformed CSV row: {e}"))
        })?;
        let field = |name: &str| -> Result<&str> {
            record.get(record_field_index(name)).ok_or_else(|| {
                ChemEngError::invalid_input("fittings", format!("row is missing column `{name}`"))
            })
        };
        let n_ld: f64 = field("n_ld")?
            .trim()
            .parse()
            .map_err(|e| ChemEngError::invalid_input("n_ld", format!("not a number: {e}")))?;
        out.push(Fitting {
...
///
/// # Errors
/// Returns [`ChemEngError::UnknownFitting`] if the id is not in the registry.
...
pub fn find(id: &str) -> Result<&'static Fitting> {
    registry()?
        .iter()
        .find(|f| f.id == id)
        .ok_or_else(|| ChemEngError::UnknownFitting { id: id.to_string() })
}
...
mod tests {
    ...
    fn unknown_id_is_an_error_not_a_silent_zero() {
        ...
        assert!(matches!(err, ChemEngError::UnknownFitting { .. }));
