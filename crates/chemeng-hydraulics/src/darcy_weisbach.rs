...
//! applied invisibly. The `chemeng pipe` CLI performs that composition and shows
//! both contributions separately.

use crate::results::DarcyWeisbachResult;
use crate::reynolds_number::regime_for;
use crate::spec_gen;
use chemeng_core::units::{DynamicViscosity, Length, MassDensity, Velocity, pascals};
...
///
/// # Errors
/// [`chemeng_core::ChemEngError::OutOfRange`] if `f`, `L`, `D`, `rho` or `v`
/// violates a hard bound in the spec - a non-positive friction factor, diameter
/// or density, a negative length or velocity.
///
/// # Example
/// ```
/// use chemeng_core::units::{kilograms_per_cubic_meter, meters, meters_per_second};
/// use chemeng_hydraulics::darcy_weisbach;
...
/// assert!(r.re.is_none());
/// assert!(r.has_warning(chemeng_core::WarningCode::RangeCheckSkipped));
