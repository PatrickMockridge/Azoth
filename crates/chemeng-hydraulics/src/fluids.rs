...
//!
//! Mirrors `chemeng.properties` on the Python side, and reads the same files -
...
use chemeng_core::units::{
    DynamicViscosity, MassDensity, kilograms_per_cubic_meter, pascal_seconds,
};
...
impl FluidTable {
    ...
    ///
    /// # Errors
    /// Returns [`ChemEngError::OutOfRange`] outside the tabulated range.
    fn interpolate(&self, temperature_c: f64, attribute: &str) -> Result<f64> {
        ...
        if temperature_c.is_nan() || temperature_c < low || temperature_c > high {
            return Err(ChemEngError::out_of_range(
                "temperature",
                temperature_c,
                format!(
        ...
        Err(ChemEngError::invalid_input(
            "temperature",
...
fn parse(name: &str, raw: &str) -> Result<FluidTable> {
    ...
    for record in reader.records() {
        let record = record.map_err(|e| {
            ChemEngError::invalid_input("fluids", format!("malformed CSV row in {name}: {e}"))
        })?;
        let get = |index: usize, column: &str| -> Result<&str> {
            record.get(index).ok_or_else(|| {
                ChemEngError::invalid_input(
                    "fluids",
        ...
        let number = |index: usize, column: &str| -> Result<f64> {
            get(index, column)?.trim().parse().map_err(|e| {
                ChemEngError::invalid_input(column, format!("not a number in {name}: {e}"))
            })
        };
...
///
/// # Errors
/// Returns [`ChemEngError::InvalidInput`] for anything but `water` or `air`. An
...
mod tests {
    ...
    fn outside_the_table_is_an_error_rather_than_an_extrapolation() {
        ...
        for temperature in [-10.0, 150.0, f64::NAN] {
            ...
            assert!(matches!(err, ChemEngError::OutOfRange { .. }), "{err}");
        }
    }
