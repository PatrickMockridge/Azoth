...
use chemeng_core::{CalcResult, ChemEngError, FlowRegime, WarningCode};
use chemeng_hydraulics::spec_gen::TestCase;
use chemeng_hydraulics::{regime_warning, reynolds_number};

const CALC_ID: &str = "hydraulics.reynolds_number";

fn call(case: &TestCase) -> chemeng_hydraulics::ReynoldsNumberResult {
    reynolds_number(
...
fn hard_bounds_raise_rather_than_warn() {
    ...
    // a `fn() -> ChemEngError` per row, which is more machinery than four
    ...
    let expect = |field: &str, err: ChemEngError| {
        assert_eq!(
            err.field(),
            Some(field),
...
fn nan_input_is_rejected() {
    ...
    .unwrap_err();
    assert!(matches!(err, ChemEngError::OutOfRange { .. }));
}
