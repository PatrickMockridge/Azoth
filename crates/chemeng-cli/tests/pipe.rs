...
//!
//! The interesting logic in `chemeng pipe` is not the argument parsing: it is
...
use chemeng_cli::pipe::{self, FlowUnit, FrictionMethod};
use chemeng_core::WarningCode;
...
#[test]
fn estimated_fitting_data_is_reported() {
    // The one thing about this tool a user most needs to see. While the registry
    // holds placeholders, every fitting result must say so.
    let result = headline();
    assert!(
        result
            .warnings
            .iter()
            .any(|w| w.code == WarningCode::EstimatedData),
        "a result built from estimated coefficients must carry ESTIMATED_DATA; got {:?}",
