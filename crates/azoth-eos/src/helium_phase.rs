...
//! ```
//!
//! Spec: `specs/models/eos/helium_phase.toml`. Pure helium, so there is no composition
//! input; the density solve is Newton from the ideal-gas guess, which finds the gas-like
//! root.
...
use crate::model_gen;
use crate::results::HeliumPhaseResult;
use crate::vega;
...
pub fn helium_phase(t: ThermodynamicTemperature, p: Pressure) -> Result<HeliumPhaseResult> {
    let spec = &model_gen::HELIUM_PHASE_SPEC;
    ...
    let tk = t.value;
    let p_pa = p.value;
    let rho = vega::solve_density(tk, p_pa);
    let props = vega::properties(tk, rho);

    Ok(HeliumPhaseResult {
        z_factor: props.z,
