...
use crate::model_gen;
use crate::results::FurstElectrolytePhaseResult;
...
pub fn furst_electrolyte_phase(
...
) -> Result<FurstElectrolytePhaseResult> {
    let spec = &model_gen::FURST_ELECTROLYTE_PHASE_SPEC;
    ...
    let (mixture, _) = crate::furst_electrolyte::furst_mixture_of(
        &names,
    ...
    Ok(FurstElectrolytePhaseResult {
        z_factor: state.z,
        ln_phi: state.ln_phi,
        warnings,
    })
