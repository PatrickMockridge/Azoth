//! The EOS-CG multi-fluid Helmholtz equation of state, as NeqSim's `EOSCGModel` carries
//! it.
//!
//! EOS-CG (Equation of State for Combustion Gases) is the 28-component GERG-style model
//! for combustion and carbon-capture streams: a mixture Helmholtz energy in reduced
//! variables `tau = Tr/T`, `delta = D/Dr` with the reducing temperature and density
//! `(Tr, Dr)` a composition average over binary interaction parameters, plus a pairwise
//! departure contribution. The coefficients live in [`crate::eos_cg_data`]; this module is
//! the arithmetic.
//!
//! Native units are NeqSim's: temperature in K, density in mol/L, pressure in kPa, and
//! `R = 8.314462618` J/(mol.K). The model layer converts to the molar SI convention.
...
/// The component names, indexed as NeqSim's EOS-CG does (index 0 is unused).
...
/// The component index for a name, or `None` if the name is not an EOS-CG component.
#[must_use]
pub fn component_index(name: &str) -> Option<usize> {
    COMPONENT_NAMES.iter().position(|c| *c == name)
}

/// The molar thermodynamic state of one EOS-CG mixture at `(T, D, x)`.
        ...
        if x[i] > EPSILON {
            ...
            for j in 4..=7 {
                if data::TH0I[i][j] < -0.5 {
                    // The `n * tau^-1 = n * T / Tc` term.
                    let term = data::N0I[i][j] * t / data::TC[i];
                    a0[0] += x[i] * term;
                    a0[1] += x[i] * (-term);
                    a0[2] += x[i] * (2.0 * term);
                } else if data::TH0I[i][j] > EPSILON {
...
/// The tau-dependent coefficient parts, `taup[i][k] = noik * tau^toik`, with the EOS-CG
...
fn t_terms(lntau: f64, x: &[f64], taup: &mut [[f64; 25]; NCOMP + 1]) {
    ...
    for i in 1..=NCOMP {
        if x[i] > EPSILON {
            let short = i > 4 && i != 15 && i != 18 && i != 20 && i != 22 && i < 23;
            for k in 1..=(data::KPOL[i] + data::KEXP[i] + data::KGAUSS[i]) as usize {
                if short {
...
pub fn alphar(itau: bool, t: f64, d: f64, x: &[f64]) -> [[f64; 4]; 4] {
    ...
    for i in 1..=NCOMP {
        ...
        for k in (1 + data::KPOL[i] + data::KEXP[i]) as usize
            ..=(data::KPOL[i] + data::KEXP[i] + data::KGAUSS[i]) as usize
        {
            let d_val = data::DOIK[i][k] as f64;
            let t_val = data::TOIK[i][k];
            let eta = data::ETA_PURE[i][k];
            let eps = data::EPSILON_PURE[i][k];
            let beta = data::BETA_PURE[i][k];
            let gam = data::GAMMA_PURE[i][k];
            let delta = del - eps;
            let theta = tau - gam;
            let exp_val = (-eta * delta * delta - beta * theta * theta).exp();
            let ndt = x[i] * delp[d_val as usize] * taup[i][k] * exp_val;

            let term_d = d_val - 2.0 * eta * del * delta;
            ar[0][1] += ndt * term_d;
            let delta_term_d_prime = -2.0 * eta * del * (2.0 * del - eps);
            let term_d2 = term_d * term_d - term_d + delta_term_d_prime;
            ar[0][2] += ndt * term_d2;

            if itau {
                ar[0][0] += ndt;
                let term_t = t_val - 2.0 * beta * tau * theta;
                ar[1][0] += ndt * term_t;
                let tau_term_t_prime = -2.0 * beta * tau * (2.0 * tau - gam);
                let term_t2 = term_t * term_t - term_t + tau_term_t_prime;
                ar[2][0] += ndt * term_t2;
                ar[1][1] += ndt * term_t * term_d;
                ar[1][2] += ndt * term_t * term_d2;
                let delta2_term_d_double_prime = -4.0 * eta * del * del;
                let delta_b_prime = 2.0 * term_d * delta_term_d_prime + delta2_term_d_double_prime;
                let term_d3 = term_d * term_d2 + delta_b_prime - 2.0 * term_d2;
                ar[0][3] += ndt * term_d3;
            }
        }
    }
...
/// `log(1/D)`, with the EOS-CG restart strategy.
#[must_use]
