//! Spec-driven tests for `eos.srk_departure`.

use azoth_core::AzothError;
use azoth_eos::spec_gen;
use azoth_eos::srk_departure;
use azoth_test_support as common;

const CALC_ID: &str = "eos.srk_departure";

const FIELDS: [&str; 4] = ["ln_phi", "h_dep_rt", "s_dep_r", "cp_dep_r"];

fn call(case: &azoth_core::spec::TestCase) -> azoth_eos::SrkDepartureResult {
    srk_departure(
...
fn field(result: &azoth_eos::SrkDepartureResult, name: &str) -> f64 {
    match name {
...
fn the_gibbs_identity_holds() {
    for a in [0.05, 0.19315231739218255, 0.4] {
        for b in [0.005, 0.02707510936404929, 0.08] {
            for z in [0.05, 0.3, 0.8, 0.95] {
                if z <= b {
                    continue;
                }
                let r = srk_departure(a, b, z, 0.715181696, 0.8).unwrap();
...
fn a_z_at_or_below_b_reduced_is_an_error_naming_the_difference() {
    for z in [0.02707510936404929, 0.01, 0.0, -1.0] {
        let err = srk_departure(0.2, 0.02707510936404929, z, 0.6, 0.8).unwrap_err();
