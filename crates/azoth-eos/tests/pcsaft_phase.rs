...
use azoth_eos::pcsaft_phase::{molar_volume, pcsaft_phase};
...
fn every_case_in_the_spec() {
    let spec = azoth_eos::model_gen::model("eos.pcsaft_phase").expect("the model is registered");
...
fn a_bad_composition_or_branch_is_refused() {
    ...
    let error = pcsaft_phase(&names, kelvins(300.0), pascals(5.0e6), &[0.9], "vapour")
        .expect_err("a composition that does not sum to one");
    assert!(error.to_string().contains("sum"), "{error}");

    let error = pcsaft_phase(&names, kelvins(300.0), pascals(5.0e6), &[1.0], "gas")
