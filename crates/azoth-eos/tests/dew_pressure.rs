...
fn the_flash_agrees_at_the_returned_pressure() {
    for (mixture, t, held) in [
        (methane_butane(), 300.0, vec![0.8, 0.2]),
        (methane_butane(), 280.0, vec![0.8, 0.2]),
        (ternary(), 300.0, vec![0.5, 0.3, 0.2]),
    ] {
        let boundary = dew_pressure(&mixture, kelvins(t), &held).unwrap();
        let flash = pt_flash(
            &mixture,
            kelvins(t),
        ...
        assert!(
            (beta - 1.0).abs() < 1e-9,
            "T={t}: beta is {beta} at the dew pressure, not one"
        );
        for (i, &yi) in held.iter().enumerate() {
            assert!(
                (flash.y[i] - yi).abs() < 1e-9,
                "T={t}: the flash's vapour {i} is {} not {yi}",
                flash.y[i]
            );
            assert!(
                (flash.x[i] - boundary.incipient[i]).abs() < 1e-9,
                "T={t}: the flash's liquid {i} is {} not {}",
                flash.x[i],
...
#[test]
fn a_vapour_with_no_dew_point_is_refused() {
    for (t, held) in [
        // Methane-rich vapours well above butane's critical temperature: no liquid
        // can condense out of these at any pressure.
        (350.0, vec![0.8, 0.2]),
        (450.0, vec![0.8, 0.2]),
        (400.0, vec![0.2, 0.3, 0.5]),
        (450.0, vec![0.6, 0.25, 0.15]),
    ] {
        ...
        let err = dew_pressure(&mixture, kelvins(t), &held)
            .expect_err(&format!("T={t} {held:?} has no dew point"));
...
fn a_genuine_dew_point_is_never_refused() {
    for (t, held) in [
        (300.0, vec![0.8, 0.2]),
        (280.0, vec![0.8, 0.2]),
        (300.0, vec![0.5, 0.3, 0.2]),
        (280.0, vec![0.6, 0.25, 0.15]),
    ] {
        let mixture = if held.len() == 3 {
            ternary()
        } else {
        ...
        let result = dew_pressure(&mixture, kelvins(t), &held)
            .unwrap_or_else(|e| panic!("T={t} {held:?} should have a dew point: {e}"));
        let max_ln_k = result
            .k
            .iter()
...
fn a_malformed_composition_is_refused() {
    ...
    for y in [
        ...
        assert!(
            dew_pressure(&mixture, kelvins(300.0), &y).is_err(),
            "y = {y:?}"
        );
    }
}
...
fn a_non_positive_temperature_is_refused() {
    ...
    for t in [0.0, -1.0] {
        assert!(
            dew_pressure(&mixture, kelvins(t), &[0.8, 0.2]).is_err(),
            "T={t}"
        );
    }
}
...
fn the_result_is_self_consistent() {
    let result = dew_pressure(&methane_butane(), kelvins(300.0), &[0.8, 0.2]).unwrap();
    let sum_x: f64 = result.incipient.iter().sum();
    assert!(
        (sum_x - 1.0).abs() < 1e-12,
        "the incipient phase sums to {sum_x}"
    );
    // `K_i = y_i / x_i` up to the normalisation, which is enforced to the residual:
    // the iteration sets `x_i = (y_i / K_i) / s` with `s - 1` at the tolerance, so
    // the identity holds relatively rather than absolutely. An absolute bound here
    // would be a bound on how large `K` is allowed to be.
    for i in 0..2 {
        let held = [0.8, 0.2][i];
        let ratio = held / result.incipient[i];
        assert!(
