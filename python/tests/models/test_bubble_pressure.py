...
from azoth.eos import Component, Mixture, bubble_pressure, mixture, pt_flash
...
def call(case: dict[str, Any]) -> BubblePressureResult:
    fluid = Mixture(
        components=tuple(
    ...
    return bubble_pressure(fluid, T=Q(case["inputs"]["T"], "K"), x=list(case["inputs"]["x"]))
...
def test_the_flash_agrees_at_the_returned_pressure() -> None:
    ...
    for fluid, t_c, held in (
        (methane_butane(), 300.0, [0.2, 0.8]),
        (methane_butane(), 330.0, [0.2, 0.8]),
        (ternary(), 320.0, [0.2, 0.3, 0.5]),
    ):
        boundary = bubble_pressure(fluid, T=Q(t_c, "K"), x=held)
        flash = pt_flash(
...
def test_a_mixture_with_no_bubble_point_is_refused() -> None:
    ...
    """
    for t_c, held in ((350.0, [0.5, 0.3, 0.2]), (340.0, [0.5, 0.3, 0.2]), (330.0, [0.5, 0.5])):
        fluid = ternary() if len(held) == 3 else methane_butane()
        with pytest.raises(OutOfRangeError) as excinfo:
            bubble_pressure(fluid, T=Q(t_c, "K"), x=held)
...
def test_a_genuine_bubble_point_is_never_refused() -> None:
    ...
    it below 1e-6.
    """
    for t_c, held in ((300.0, [0.2, 0.8]), (280.0, [0.2, 0.8]), (320.0, [0.2, 0.3, 0.5])):
        fluid = ternary() if len(held) == 3 else methane_butane()
        result = bubble_pressure(fluid, T=Q(t_c, "K"), x=held)
...
def test_a_single_component_is_refused_and_points_at_the_right_calc() -> None:
    ...
    with pytest.raises(InvalidInputError) as excinfo:
        bubble_pressure(propane, T=Q(300.0, "K"), x=[1.0])
...
def test_a_malformed_composition_is_refused() -> None:
    ...
    for x in ([0.2, 0.9], [0.2, 0.7], [0.6, -0.6], [0.2, 0.8, 0.0]):
        with pytest.raises(InvalidInputError):
            bubble_pressure(fluid, T=Q(300.0, "K"), x=x)


def test_the_result_is_self_consistent() -> None:
    result = bubble_pressure(methane_butane(), T=Q(300.0, "K"), x=[0.2, 0.8])
