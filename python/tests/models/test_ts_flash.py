"""Tests for the ``eos.ts_flash`` model.
...
from azoth.eos import IdealGasModel, Mixture, component, mixture, ts_flash
from azoth.eos.reference._flash_property import property_at

MODEL_ID = "eos.ts_flash"
Q = ureg.Quantity
...
def test_spec_case(case: dict[str, Any]) -> None:
    result = ts_flash(**h.model_kwargs(SPEC, case["inputs"]))
...
def test_the_round_trip_inverts_the_property() -> None:
    ...
    z = [0.6, 0.4]
    target, _ = property_at(fluid, ideal_gas, 300.0, 1.0e6, z, "s")
    result = ts_flash(fluid, ideal_gas, Q(300.0, "K"), Q(target, "J/(mol*K)"), z)
