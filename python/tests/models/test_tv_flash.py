"""Tests for the ``eos.tv_flash`` model.

The spec's cases pin the answers; the round-trip below proves the model *inverts* the
volume without knowing the pressure - compute ``V(P)`` at a pressure the test chose, ask
for it back, and require the pressure that comes out to be the one that went in.
"""
...
from azoth.eos import IdealGasModel, Mixture, component, mixture, tv_flash
...
def test_spec_case(case: dict[str, Any]) -> None:
    result = tv_flash(**h.model_kwargs(SPEC, case["inputs"]))
...
def test_the_round_trip_inverts_the_volume() -> None:
    ...
    result = tv_flash(fluid, ideal_gas, Q(300.0, "K"), Q(volume, "m**3/mol"), z)
    h.assert_close(result.P.to("Pa").magnitude, 1.0e6, 1e-5, "P")
