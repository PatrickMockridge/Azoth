...
conversion applied uniformly and wrongly, producing 286 rows that all agree with
...
def test_the_databank_is_populated() -> None:
    ...
    assert len(available_components()) > 100, (
        ...
        f"roughly 286. Regenerate with tools/gen_databank.py."
    )
...
def test_the_reference_state_column_is_carried_and_never_blank() -> None:
    ...
    exactly the 286 rows the databank keeps, NeqSim's `COMP.csv` fills it on every one -
    `solvent` on 233, `solute` on 51, and the literal `0.0` on the two acids. A column
