...
Rachford-Rice is normally an equation to *solve*: for N components it is nonlinear
in ``beta`` and needs iteration. For two components it is linear, and multiplying
through by the two denominators clears them:
...
Outside that interval the feed is single phase and the solution is the tangent-plane
value rather than a phase split. It is still returned, carrying
