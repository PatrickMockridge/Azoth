...
(:func:`azoth.eos.reference._mixture_state.criticality_matrix`) to zero at fixed volume,
and an outer solve drives the cubic form along that eigenvalue's eigenvector to zero.
Both conditions are evaluated at constant temperature and volume, so the state is
``(T, V)`` throughout: at a critical point the cubic is degenerate, and the pressure is
taken from the volume through the explicit equation of state rather than from a root.
