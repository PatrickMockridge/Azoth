...
Spec: ``specs/models/eos/unifac_umrpru_activity_coefficients.toml``. A *direct* model:
...
from azoth.core.result import UnifacUmrpruActivityCoefficientsResult
from azoth.core.units import Q, input_to_si
from azoth.core.warnings import Warning
from azoth.eos.components import UnifacUmrpruParameters

MODEL_ID = "eos.unifac_umrpru_activity_coefficients"

#: The temperature the UMR-PRU interaction is fitted about. A constant of the model
#: rather than of the state, and the one place it differs from UNIFAC-PSRK besides the
#: tables it reads.
...
def unifac_umrpru_activity_coefficients(
    ...
    Args:
        params: the resolved UNIFAC-UMR-PRU parameters, by component.
    ...
    Example:
        >>> import azoth
        >>> from azoth.eos.components import unifac_umrpru_parameters
        >>> q = azoth.ureg.Quantity
        >>> r = unifac_umrpru_activity_coefficients(
        ...     unifac_umrpru_parameters(["methanol", "water"], "umr"),
        ...     q(298.15, "K"), [0.5, 0.5],
        ... )
        >>> round(r.gamma[0], 12)
        1.115681506247
