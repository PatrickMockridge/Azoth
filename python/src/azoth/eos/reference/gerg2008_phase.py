...
Spec: ``specs/models/eos/gerg2008_phase.toml``. The 21-component wide-range Helmholtz
...
from azoth.core.result import Gerg2008PhaseResult
from azoth.core.units import Q, from_si, input_to_si
from azoth.core.warnings import Warning
from azoth.eos.reference import _gerg2008_data as data

MODEL_ID = "eos.gerg2008_phase"
...
def _composition(components: list[str], z: list[float]) -> list[float]:
    ...
    for name, fraction in zip(components, z, strict=True):
        ...
        if index is None:
            raise InvalidInputError("components", f"unknown GERG-2008 component {name!r}")
        x[index] += fraction
    return x
...
def _alphar(itau: bool, t: float, d: float, x: list[float]) -> list[list[float]]:
    ...
    for i in range(1, NCOMP + 1):
        if x[i] > _EPSILON:
            short = i > 4 and i != 15 and i != 18 and i != 20
            for k in range(1, data.KPOL[i] + data.KEXP[i] + 1):
                if short:
...
def gerg2008_phase(T: Q, P: Q, components: list[str], z: list[float]) -> Gerg2008PhaseResult:
    """The GERG-2008 phase state at a temperature, pressure and composition.

    Args:
        T: absolute temperature.
        P: absolute pressure.
        components: the GERG-2008 component names, one per entry.
