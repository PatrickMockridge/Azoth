...
class DatabankEntry:
    ...
    **Those wider fields are optional, and a keycard is why.** A keycard supplies the
    ...
    #: applied where the polynomial is read.
    ...
    liquid_density_coefs: tuple[float, float, float, float]
    ...
    def component(self, alpha: str | None = None) -> Component:
        ...
        return Component(
            ...
            liquid_density_coefs=self.liquid_density_coefs,
...
def _table() -> dict[str, DatabankEntry]:
    ...
    for row in _rows(find(COMPONENTS_CSV).read_text(encoding="utf-8")):
        ...
        entries[name] = DatabankEntry(
            ...
            liquid_density_coefs=tuple(  # type: ignore[arg-type]
                float(row[f"liquiddensitycoefs{k}"]) for k in range(1, 5)
            ),
            alpha_params={
...
def entry(name: str, *, card: keycard.Keycard | None = None) -> DatabankEntry:
    ...
    if base is None:
        ...
        return DatabankEntry(
            ...
            liquid_density_coefs=(0.0, 0.0, 0.0, 0.0),
