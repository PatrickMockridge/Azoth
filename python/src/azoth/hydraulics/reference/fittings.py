...
Reading the same bytes is not the same as being checked against each other, and the
two *are* checked against each other: ``tests/test_data_agreement.py`` compares every
parsed coefficient row by row against the Rust registry, and compares the bytes this
side reads against the bytes the Rust core embedded. The second check is the stronger
one - parsed values can agree across two different files, and a stale copy bundled
into a wheel would look exactly like that.
