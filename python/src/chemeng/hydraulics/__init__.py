...
``chemeng pipe`` CLI rather than by a calc of its own, because the two losses are
...
:mod:`chemeng.hydraulics.reference` otherwise. Both are always reachable - see
:func:`chemeng.backends` and :func:`chemeng.use_backend`.
...
def reynolds_number(rho: Q, v: Q, D: Q, mu: Q) -> ReynoldsNumberResult:
    ...
    See :func:`chemeng.hydraulics.reference.reynolds_number` for the full
...
def friction_factor_colebrook(re: float, relative_roughness: float) -> ColebrookResult:
    ...
    See :func:`chemeng.hydraulics.reference.friction_factor_colebrook`.
    """
...
def friction_factor_swamee_jain(re: float, relative_roughness: float) -> SwameeJainResult:
    ...
    See :func:`chemeng.hydraulics.reference.friction_factor_swamee_jain`.
    """
...
def crane_k_factors(fittings: Sequence[str], f_t: float) -> KFactorsResult:
    ...
    See :func:`chemeng.hydraulics.reference.crane_k_factors`.
    """
...
def darcy_weisbach(
    ...
    See :func:`chemeng.hydraulics.reference.darcy_weisbach`.
    """
