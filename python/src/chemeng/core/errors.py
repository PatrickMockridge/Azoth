...
Mirrors `chemeng_core::error` in Rust. The split between an error and a warning
...
* **Warning** (`chemeng.core.warnings`) - the calculation is well defined but
...
heard of chemeng can still catch ``ValueError`` or ``RuntimeError`` and get
something sensible.
"""
...
class ChemEngError(Exception):
    ...
    Deliberately does not inherit from a builtin, so ``except ChemEngError`` is a
...
class InvalidInputError(ChemEngError, ValueError):
...
class OutOfRangeError(ChemEngError, ValueError):
...
class SolverNotConvergedError(ChemEngError, RuntimeError):
...
class UnitMismatchError(ChemEngError, TypeError):
    ...
    on purpose: ``chemeng.core.errors`` must not import pint, and pint's
...
class UnknownFittingError(ChemEngError, LookupError):
...
class UnverifiedCalculationError(ChemEngError):
...
class PropertyUnavailableError(ChemEngError, LookupError):
