...
backend in use can be queried, and ``CHEMENG_REQUIRE_RUST=1`` turns "the
...
_ENV_BACKEND = "CHEMENG_BACKEND"

#: Env var that makes a missing extension fatal.
_ENV_REQUIRE_RUST = "CHEMENG_REQUIRE_RUST"
...
def _load_extension() -> ModuleType | None:
    """Import ``chemeng._core`` if it was built.
...
def select() -> Backend:
    ...
    if env in ("python", "rust"):
        if env == "rust" and _EXTENSION is None:
            raise RustBackendUnavailableError(
                f"{_ENV_BACKEND}=rust but chemeng._core is not built. "
    ...
    if os.environ.get(_ENV_REQUIRE_RUST) == "1" and _EXTENSION is None:
        raise RustBackendUnavailableError(
            f"{_ENV_REQUIRE_RUST}=1 but chemeng._core is not built. "
...
def resolve(calc_id: str) -> Callable[..., Any]:
    ...
    reference: Callable[..., Any] = getattr(
        importlib.import_module(f"chemeng.hydraulics.reference.{function_name}"),
        function_name,
    )
