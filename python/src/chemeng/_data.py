...
depends on how chemeng was obtained:
...
def _packaged(name: str) -> Path | None:
    ...
    try:
        candidate = resources.files("chemeng") / "_data" / name
...
def find(relative: str) -> Path:
    ...
    raise DataFileNotFoundError(
        f"could not locate {relative!r}. This build of chemeng resolves data files "
        f"from a source checkout; if it was installed as a wheel, the data was not "
        f"packaged - see chemeng/_data.py."
    )
